...
static int
_mask_and_sort(
    int ninputs, int index, int fill, npy_float64 **inputs, npy_uint8 **masks,
...
static npy_float64
_inner_median(int goodpix, int nlow, int nhigh, int ninputs, npy_float64 temp[MAX_ARRAYS])
{
    npy_float64 median;
    ...
    if (medianpix <= 0) {
        ...
        if (medianpix % 2) /* odd */ {
            ...
            median = (temp[midpoint + nlow] + temp[midpoint + nlow - 1]) / 2.0;
        }
    }
    return median;
}
...
static npy_float64
_inner_old_median(int goodpix, int nlow, int nhigh, int ninputs, npy_float64 temp[MAX_ARRAYS])
{
    npy_float64 median;
    int midpoint, medianpix = goodpix - nhigh - nlow;
    if (medianpix <= 0) {
...
static npy_float64
_inner_average(int goodpix, int nlow, int nhigh, int ninputs, npy_float64 temp[MAX_ARRAYS])
{
    npy_float64 average;
...
static npy_float64
_inner_minimum(int goodpix, int nlow, int nhigh, int ninputs, npy_float64 temp[MAX_ARRAYS])
{
...
static int
_combine(
    combiner f, int dim, int maxdim, int ninputs, int nlow, int nhigh, int fillval,
    PyArrayObject *inputs[], PyArrayObject *masks[], PyArrayObject *output)
{
    ...
    if (dim == maxdim - 1) {
        ...
        for (j = 0; j < cols; j++) {
            int goodpix =
                _mask_and_sort(ninputs, j, fillval, tinputs, masks ? tmasks : NULL, sorted);
            if (fillval == 1) {
                fillval = ninputs;
            }
        ...
        for (i = 0; i < inputs[0]->dimensions[dim]; i++) {
            ...
            _combine(f, dim + 1, maxdim, ninputs, nlow, nhigh, fillval, inputs, masks, output);
            for (j = 0; j < ninputs; j++) {
                inputs[j]->data -= inputs[j]->strides[dim] * i;
                if (masks) {
                    masks[j]->data -= masks[j]->strides[dim] * i;
...
static fmapping functions[] = {
    {"median", _inner_median},  {"average", _inner_average},  {"minimum", _inner_minimum},
    {"imedian", _inner_median}, {"iaverage", _inner_average},
};
...
_Py_combine(PyObject *obj, PyObject *args, PyObject *kw)
{
    ...
    char *keywds[] = {"arrays", "output", "nlow", "nhigh", "badmasks", "kind", NULL};
    char *kind;
    combiner f;
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kw, "OO|iiOs:combine", keywds, &arrays, &output, &nlow, &nhigh, &badmasks,
            &kind)) {
        return NULL;
    }
    ...
    if (narrays < 0) {
        return PyErr_Format(PyExc_TypeError, "combine: arrays is not a sequence");
    }
    if (narrays > MAX_ARRAYS) {
        return PyErr_Format(PyExc_TypeError, "combine: too many arrays.");
    }
    ...
    for (i = 0; i < narrays; i++) {
        ...
        arr[i] = (PyArrayObject *) PyArray_FROM_OTF(a, NPY_FLOAT64, NPY_ARRAY_INOUT_ARRAY2);
        if (!arr[i]) {
            return NULL;
        }
        Py_DECREF(a);
        if (badmasks != Py_None) {
            ...
            bmk[i] = (PyArrayObject *) PyArray_FROM_OTF(a, NPY_UINT8, NPY_ARRAY_IN_ARRAY);
            if (!bmk[i]) {
                return NULL;
            }
            Py_DECREF(a);
    ...
    toutput = (PyArrayObject *) PyArray_FROM_OTF(output, NPY_FLOAT64, NPY_ARRAY_INOUT_ARRAY2);
    if (!toutput) {
        return NULL;
    }
    ...
    if (!f) {
        return PyErr_Format(PyExc_ValueError, "Invalid combination function.");
    }

    if (_combine(
            f, 0, arr[0]->nd, narrays, nlow, nhigh, fillval, arr, // codespell:ignore nd
