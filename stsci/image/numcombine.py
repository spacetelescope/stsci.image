...
#                     more values are not being eliminated than are available in
#                     the image stack. Also, a list of masks can now be accepted
#                     as input and applied to the input image list in addition
...
class numCombine:
    ...
    def __init__(
        ...
        nkeep=1,  # Minimum number of pixels to keep for a valid computation
        upper=None,  # Throw out values >= to upper in a median calculation
        lower=None,  # Throw out values < lower in a median calculation
    ):
        warnings.warn(
            "The 'numCombine' class is deprecated and may be removed"
        ...
        self.combArrObj = np.zeros(
            self.__arrObjectList[0].shape, dtype=self.__arrObjectList[0].dtype
        )
        ...
        if self.__combinationType.lower() == "median":
            ...
            self.__combinationType.lower() == "iaverage"
            or self.__combinationType.lower() == "imean"
        ):
            self._iaverage()
    ...
    def _createMaskList(self):
        ...
        if (self.__lower is not None) or (self.__upper is not None):
            __tmpMaskList = []
            for imgobj in self.__arrObjectList:
                __tmpMask = image.threshold(imgobj, low=self.__lower, high=self.__upper)
                __tmpMaskList.append(__tmpMask)
        else:
            __tmpMaskList = None

        # Combine the threshold masks with the input masks
        if (self.__arrMaskList is None) and (__tmpMaskList is None):
            self.__masks = None
        elif (self.__arrMaskList is None) and (__tmpMaskList is not None):
            self.__masks = __tmpMaskList
        elif (self.__arrMaskList is not None) and (__tmpMaskList is None):
    ...
    def _minimum(self):
        ...
        try:
            ...
            for imgobj in self.__arrObjectList:
                _newMax = imgobj.max()
                _maxValue = max(_maxValue, _newMax)

            if self.__arrMaskList is not None:
                tmpList = []
                ...
                for imgobj in range(len(self.__arrObjectList)):
                    tmp = np.where(
                        self.__arrMaskList[imgobj] == 1,
                        _maxValue + 1,
                        self.__arrObjectList[imgobj],
                    )
                    tmpList.append(tmp)
            else:
                tmpList = list(self.__arrObjectList)
...
def num_combine(
    data, masks=None, combination_type="median", nlow=0, nhigh=0, upper=None, lower=None
):
    ...
    if data.shape[0] - nlow - nhigh < 1:
        raise ValueError("Rejecting all pixels due to large 'nval' and/or 'nhigh'!")
    ...
    if lower is not None or upper is not None:
        thmasks = image.threshold(data, low=lower, high=upper)
        if masks is None:
            masks = thmasks
        else:
    ...
    if combination_type == "median":
        image.median(data, comb_arr, comb_arr.dtype, nlow=nlow, nhigh=nhigh, badmasks=masks)
    elif combination_type == "imedian":
        image.imedian(data, comb_arr, comb_arr.dtype, nlow=nlow, nhigh=nhigh, badmasks=masks)
    elif combination_type in ["iaverage", "imean"]:
        image.iaverage(data, comb_arr, comb_arr.dtype, nlow=nlow, nhigh=nhigh, badmasks=masks)
    elif combination_type == "mean":
        image.average(data, comb_arr, comb_arr.dtype, nlow=nlow, nhigh=nhigh, badmasks=masks)
    elif combination_type == "sum":
        np.sum(data, axis=0, out=comb_arr)
    elif combination_type == "minimum":
        image.minimum(data, comb_arr, comb_arr.dtype, nlow=nlow, nhigh=nhigh, badmasks=masks)
    else:
        comb_arr.fill(0)
        print("Combination type not supported!!!")
